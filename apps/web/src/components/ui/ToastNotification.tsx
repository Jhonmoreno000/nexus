    ...
    <div className="fixed bottom-5 right-5 z-50 flex flex-col gap-2.5 max-w-sm pointer-events-none select-none">
      {toasts.map((toast) => {
        return (
          <div
            key={toast.id}
            className="pointer-events-auto p-4 rounded-xl bg-[#09152b] border border-[#00e5ff]/50 shadow-2xl flex items-start gap-3 text-xs animate-in slide-in-from-bottom-5 duration-200"
          >
            ...
            <div className="flex-1 space-y-0.5">
              <div className="flex items-center justify-between">
                <span className="font-bold text-white text-xs tracking-wide">{toast.title}</span>
                {toast.xp && (
                  <span className="px-1.5 py-0.5 rounded bg-[#00e5ff]/20 text-[#00e5ff] font-mono font-bold text-[10px]">
                    +{toast.xp} XP
                  </span>
                )}
              </div>
